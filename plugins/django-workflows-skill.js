/**
 * Django Workflows Skill Plugin (Skill-Only)
 *
 * This is a "skill-only" plugin that provides NO hooks or APIs.
 * Instead, it teaches LLM agents how to orchestrate complex Django integration workflows
 * by combining existing plugins (table, form-schema, agent-commands, state-tracker, fixi-agent).
 *
 * @module plugins/django-workflows-skill
 *
 * Purpose:
 * - Teach agents Django-FixiPlug integration patterns
 * - Combine multiple plugins into cohesive workflows
 * - Provide Django-specific best practices
 * - Guide agents through CRUD operations with dj-fixi
 *
 * @example
 * // This plugin provides no hooks - it only teaches agents via skill metadata
 * // The agent learns to coordinate: fx-attributes + tables + forms + state management
 */

export default function djangoWorkflowsSkill(ctx) {
  // NO hooks registered - this is a pure skill plugin
  // The skill metadata below teaches agents how to use existing plugins together
  ...
  return {
    skill: {
      ...
      instructions: `# Django Integration Workflows Skill
