/**
 * FixiPlug Reactive UI Patterns Skill Plugin (Skill-Only)
 *
 * This is a pure skill plugin - it contains NO executable code or hooks.
 * It exists solely to teach LLM agents how to orchestrate existing FixiPlug
 * plugins (fixi-agent + state-tracker) to build reactive user interfaces.
 *
 * This demonstrates the skill-only plugin pattern where pedagogical metadata
 * is separated from implementation code.
 *
 * @module plugins/reactive-ui-patterns-skill
 */

/**
 * Reactive UI Patterns Skill Plugin Factory
 * @param {Object} ctx - Plugin context (unused in skill-only plugins)
 * @returns {Object} Skill metadata
 */
export default function reactiveUiPatternsSkill(ctx) {
  // Note: No ctx.on() calls - this plugin registers ZERO hooks
  // It's purely pedagogical metadata
  ...
  return {
    skill: {
      ...
      instructions: `# Reactive UI Patterns with FixiPlug
...
`,
      ...
      // References to plugins this skill orchestrates
      references: [
        'stateTrackerPlugin',
        'fixiAgentPlugin',
        'introspectionPlugin'
