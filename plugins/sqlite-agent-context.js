/**
 * SQLite Agent Context Plugin
 *
 * Enables LLM agents to detect their capabilities, manage context, and format responses appropriately.
 * Integrates with SQLite Extensions Framework for agent context management.
 *
 * @module plugins/sqlite-agent-context
 */
...
1.0.0 - Initial release`,
  ...
  async setup(ctx) {
    ...
    async function getService() {
      if (!service) {
        try {
          ...
          console.log('[sqliteAgentContext] Service initialized');
        } catch (error) {
          console.error('[sqliteAgentContext] Failed to initialize service:', error.message);
          throw error;
        }
      }
      return service;
    ...
    /**
     * Hook: Detect agent type
     */
    ctx.on('sqlite.context.detect', async (params) => {
      try {
        const svc = await getService();
        const result = await svc.detectAgentType({
          hints: params?.hints
        });
        return result;
      } catch (error) {
        console.error('[sqliteAgentContext] Error in sqlite.context.detect:', error);
        throw error;
      }
    });

    /**
     * Hook: Get agent capabilities
     */
    ctx.on('sqlite.context.capabilities', async (params) => {
      try {
        const svc = await getService();
        const result = await svc.getAgentCapabilities({
          agentType: params.agentType,
          includeDetails: params.includeDetails || false
        });
        return result;
      } catch (error) {
        console.error('[sqliteAgentContext] Error in sqlite.context.capabilities:', error);
        throw error;
      }
    });

    /**
     * Hook: Calculate token budget
     */
    ctx.on('sqlite.context.token_budget', async (params) => {
      try {
        const svc = await getService();
        const result = await svc.getTokenBudget({
          agentType: params.agentType,
        ...
        console.error('[sqliteAgentContext] Error in sqlite.context.token_budget:', error);
        throw error;
      }
    });

    /**
     * Hook: Format response for agent
     */
    ctx.on('sqlite.context.format_response', async (params) => {
      try {
        const svc = await getService();
        const result = await svc.formatResponse({
        ...
        console.error('[sqliteAgentContext] Error in sqlite.context.format_response:', error);
        throw error;
      }
    });
    ...
    ctx.on('unload', async () => {
      if (service) {
        try {
          ...
          console.log('[sqliteAgentContext] Service shutdown complete');
        } catch (error) {
          console.error('[sqliteAgentContext] Error during shutdown:', error);
        }
        service = null;
      }
    });

    console.log('[sqliteAgentContext] Plugin initialized');
  }
};
