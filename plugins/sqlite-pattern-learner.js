/**
 * SQLite Pattern Learner Plugin
 *
 * Enables LLM agents to learn from database query patterns and get optimized recommendations.
 * Integrates with SQLite Extensions Framework for pattern learning capabilities.
 *
 * @module plugins/sqlite-pattern-learner
 */
...
1.0.0 - Initial release`,
    tags: ['database', 'patterns', 'learning', 'optimization', 'sqlite'],
    version: '1.0.0'
  },
  ...
  async setup(ctx) {
    ...
    async function getService() {
      if (!service) {
        try {
          ...
          console.log('[sqlitePatternLearner] Service initialized');
        } catch (error) {
          console.error('[sqlitePatternLearner] Failed to initialize service:', error.message);
          throw error;
        }
      }
      return service;
    ...
    /**
     * Hook: Get pattern recommendations
     */
    ctx.on('sqlite.patterns.get', async (params) => {
      try {
        ...
        console.error('[sqlitePatternLearner] Error in sqlite.patterns.get:', error);
        throw error;
      }
    });
    ...
    ctx.on('sqlite.patterns.find_similar', async (params) => {
      try {
        const svc = await getService();
        const result = await svc.findSimilarPatterns({
          description: params.description,
        ...
        console.error('[sqlitePatternLearner] Error in sqlite.patterns.find_similar:', error);
        throw error;
      }
    });

    /**
     * Hook: Get pattern statistics
     */
    ctx.on('sqlite.patterns.statistics', async (params) => {
      try {
        const svc = await getService();
        const result = await svc.getPatternStatistics({
          domain: params.domain,
          timeRange: params.timeRange || 'week',
          sortBy: params.sortBy || 'usage'
        });
        return result;
      } catch (error) {
        console.error('[sqlitePatternLearner] Error in sqlite.patterns.statistics:', error);
        throw error;
      }
    });
    ...
    ctx.on('sqlite.patterns.record', async (params) => {
      try {
        const svc = await getService();
        const result = await svc.recordPattern({
        ...
        console.error('[sqlitePatternLearner] Error in sqlite.patterns.record:', error);
        throw error;
      }
    });
    ...
    ctx.on('unload', async () => {
      if (service) {
        try {
          ...
          console.log('[sqlitePatternLearner] Service shutdown complete');
        } catch (error) {
          console.error('[sqlitePatternLearner] Error during shutdown:', error);
        }
        service = null;
      }
    });

    console.log('[sqlitePatternLearner] Plugin initialized');
  }
};
