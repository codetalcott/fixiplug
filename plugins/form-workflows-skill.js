/**
 * Form Workflows Skill Plugin (Skill-Only)
 *
 * This is a "skill-only" plugin that provides NO hooks or APIs.
 * Instead, it teaches LLM agents how to handle complex multi-step form workflows,
 * wizard forms, conditional forms, validation orchestration, and form state management.
 *
 * @module plugins/form-workflows-skill
 *
 * Purpose:
 * - Teach agents multi-step form patterns
 * - Handle wizard-style forms
 * - Manage conditional form logic
 * - Orchestrate validation across steps
 * - Coordinate form + state + agent-commands plugins
 *
 * @example
 * // This plugin provides no hooks - it only teaches agents via skill metadata
 * // The agent learns to coordinate: form-schema + agent-commands + state-tracker
 */

export default function formWorkflowsSkill(ctx) {
  // NO hooks registered - this is a pure skill plugin
  // The skill metadata below teaches agents form workflow patterns
  ...
  return {
    skill: {
      ...
      instructions: `# Multi-Step Form Workflows Skill
